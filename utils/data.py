...
def load_PPMI_data(type_i='metabolite'):
    types = {'metabolite': 0,
         'protein': 1}
    ...
    for metapath in PPMI_expected_metapaths[types[type_i]]:
        ...
        in_file = open(metapaths_metabolites_file, 'r')
        adjlist_metapath = [line.strip() for line in in_file]
        adjlist_metapath = adjlist_metapath[3:]
        in_file.close()
        adjlists.append(adjlist_metapath)

        filename = 'edges_' + '-'.join(map(str, metapath)) + '_idx.pickle'
        edges_file = du.get_file_path(data_dir, 'MAGNN', 'preprocessed', filename)
        in_file = open(edges_file, 'rb')
        idx_metapath = pickle.load(in_file)
        in_file.close()
        edge_metapath_indices_list.append(idx_metapath)
    ...
    features_metabolites = scipy.sparse.load_npz(features_metabolites_file).toarray()
    features_protein = scipy.sparse.load_npz(features_proteins_file).toarray()
    features_list = [features_metabolites, features_protein]

    adjM = scipy.sparse.load_npz(adjM_file)
    type_mask = np.load(node_types_file)
    labels = np.load(labels_file)
    train_val_test_idx = np.load(val_train_test_idx_file)
    
    return adjlists, edge_metapath_indices_list, features_list, adjM, type_mask, labels, train_val_test_idx
